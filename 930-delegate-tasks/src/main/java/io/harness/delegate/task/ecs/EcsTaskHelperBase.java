...
public class EcsTaskHelperBase {
  ...
  public List<ServerInstanceInfo> getEcsServerInstanceInfos(
    ...
    List<EcsTask> ecsTasks = ecsCommandTaskNGHelper.getRunningEcsTasks(ecsInfraConfig.getAwsConnectorDTO(),ecsInfraConfig.getCluster()
    ,deploymentReleaseData.getServiceName(), ecsInfraConfig.getRegion());
    if(ecsTasks!=null && ecsTasks.size()>0) {
      return EcsTaskToServerInstanceInfoMapper.toServerInstanceInfoList(ecsTasks, ecsInfraConfig.getInfraStructureKey(),
