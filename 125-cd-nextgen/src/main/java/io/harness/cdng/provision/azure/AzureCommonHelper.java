...
public class AzureCommonHelper {
  ...
  public GitFetchFilesConfig getTemplateGitFetchFileConfig(
      Ambiance ambiance, AzureCreateARMTemplateFile azureCreateTemplateFile, String deploymentType) {
  ...
  boolean isTemplateStoredOnGit(AzureCreateARMTemplateFile azureCreateTemplateFileSpec) {
    return ManifestStoreType.isInGitSubset(azureCreateTemplateFileSpec.getStore().getSpec().getKind());
  }

  AzureCreateARMPassThroughData getAzureCreatePassThroughData(
      AzureCreateARMStepConfigurationParameters stepConfiguration) {
    boolean hasGitFiles =
        hasGitStoredParameters(stepConfiguration) || isTemplateStoredOnGit(stepConfiguration.getTemplateFile());

    return AzureCreateARMPassThroughData.builder().hasGitFiles(hasGitFiles).build();
  }
  ...
  List<GitFetchFilesConfig> getParametersGitFetchFileConfigs(
      Ambiance ambiance, AzureCreateARMStepConfigurationParameters stepConfiguration) {
