...
public class AzureCreateStepHelper {
    ...
    public TaskChainResponse startChainLink(AzureCreateStepExecutor azureCreateStepExecutor, Ambiance ambiance, StepElementParameters stepElementParameters){
        ...
        List<GitFetchFilesConfig> gitFetchFilesConfigs = getParametersGitFetchFileConfigs(ambiance, stepConfigurationParameters);

        AzureCreateTemplateFileSpec azureCreateTemplateFileSpec = stepConfigurationParameters.getAzureDeploymentType().getTemplateSpecs().getSpec();
        ...
        if (Objects.equals(azureCreateStepParameters.getConfiguration().getAzureDeploymentType().getType(),AzureDeploymentTypes.ARM)){
            AzureARMDeploymentSpec azureARMDeploymentSpec = (AzureARMDeploymentSpec) azureCreateStepParameters.getConfiguration().getAzureDeploymentType();
            if(Objects.equals(azureARMDeploymentSpec.getTemplateSpecs().getType(), AzureCreateTemplateFileTypes.Inline)){
                templateBody = ((AzureInlineTemplateFileSpec)azureARMDeploymentSpec.getTemplateFile().getSpec()).getTemplateBody().getValue();
            }

            if (Objects.equals(azureARMDeploymentSpec.getParameters().getType(), AzureARMParametersFileTypes.Inline)) {
                parametersBody = (((AzureInlineParametersFileSpec) azureARMDeploymentSpec.getParameters().getSpec()).getParameterBody().getValue());
            }

        }
        populatePassThroughData(passThroughData, templateBody, parametersBody,null ,null ,null );
        AzureTaskNGParameters azureARMTaskNGParameters = getAzureTaskNGParams(ambiance, stepElementParameters, (AzureConnectorDTO) connectorDTO.getConnectorConfig(),passThroughData);

    return azureCreateStepExecutor.executeCreateTask(ambiance, stepElementParameters, azureARMTaskNGParameters, passThroughData);
    }

    private AzureTaskNGParameters getAzureTaskNGParams(Ambiance ambiance, StepElementParameters stepElementParameters, AzureConnectorDTO connectorConfig, AzureCreatePassThroughData passThroughData) {
        AzureCreateStepParameters azureCreateStepParameters = (AzureCreateStepParameters) stepElementParameters.getSpec();
        AzureCreateStepConfigurationParameters stepConfigurationParameters = azureCreateStepParameters.getConfiguration();
        if (Objects.equals(stepConfigurationParameters.getAzureDeploymentType().getType(), AzureDeploymentTypes.ARM)) {
            ...
            AzureBluePrintDeploymentSpec specs =  (AzureBluePrintDeploymentSpec) stepConfigurationParameters.getAzureDeploymentType();
    ...
    private void populatePassThroughData(AzureCreatePassThroughData passThroughData, String templateBody, String parametersBody, String blueprintBody, String assignmentBody, Map<String, String> artifacts) {
    ...
    private TaskChainResponse getGitFetchFileTaskChainResponse(Ambiance ambiance, List<GitFetchFilesConfig> gitFetchFilesConfigs, StepElementParameters stepElementParameters, AzureCreatePassThroughData passThroughData) {
    ...
    private List<GitFetchFilesConfig> getParametersGitFetchFileConfigs(
        ...
        if (Objects.equals(stepConfiguration.getAzureDeploymentType().getType(), AzureDeploymentTypes.ARM)) {
            ...
            if (specs.getParameters()!= null && Objects.equals(specs.getParameters().getType(), AzureARMParametersFileTypes.Remote)) {
                ...
                return new ArrayList<>(Collections.singletonList(GitFetchFilesConfig.builder()
                        .manifestType("Azure Parameters")
                        .identifier(PARAMETERS_FILE_IDENTIFIER)
                        .gitStoreDelegateConfig(getGitStoreDelegateConfig(fileSpec.getStore().getSpec(), ambiance,AzureDeploymentTypes.ARM ))
                        .build()));
            }
        }
    ...
    private GitFetchFilesConfig getTemplateGitFetchFileConfig(
            Ambiance ambiance, AzureCreateStepConfigurationParameters stepConfiguration) {
        AzureRemoteTemplateFileSpec azureCreateTemplateFileSpec = (AzureRemoteTemplateFileSpec) stepConfiguration.getAzureDeploymentType().getTemplateSpecs().getSpec();
        if (Objects.equals(stepConfiguration.getAzureDeploymentType().getType(), AzureDeploymentTypes.ARM)) {
            return GitFetchFilesConfig.builder()
                    .manifestType("Azure Template")
                    .identifier(TEMPLATE_FILE_IDENTIFIER)
                    .gitStoreDelegateConfig(
                            getGitStoreDelegateConfig(azureCreateTemplateFileSpec.getStore().getSpec(), ambiance, AzureDeploymentTypes.ARM))
                    .build();
        } else {
            return GitFetchFilesConfig.builder()
                    ...
                    .gitStoreDelegateConfig(
                            getGitStoreDelegateConfig(azureCreateTemplateFileSpec.getStore().getSpec(), ambiance,AzureDeploymentTypes.BLUEPRINT ))
                    .build();
        }
    }
    ...
    private boolean hasGitStoredParameters(AzureCreateStepConfigurationParameters stepConfigurationParameters) {
        if (Objects.equals(stepConfigurationParameters.getAzureDeploymentType().getType(), AzureDeploymentTypes.ARM)) {
            ...
            return spec.getParameters() != null &&
                    ...
                    ManifestStoreType.isInGitSubset(((AzureRemoteParametersFileSpec)spec.getParameters().getSpec()).getStore().getSpec().getKind());
        }
        return false;
    }

    TaskChainResponse handleGitFetchResponse(AzureCreateStepExecutor azureCreateStepExecutor,
                                             Ambiance ambiance,
        ...
        if (Objects.equals(stepConfigurationParameters.getAzureDeploymentType().getType(), AzureDeploymentTypes.ARM)) {
            if (filesFromMultipleRepo.get(PARAMETERS_FILE_IDENTIFIER) != null) {
                ...
                AzureARMDeploymentSpec spec = (AzureARMDeploymentSpec) stepConfigurationParameters.getAzureDeploymentType();
                if (spec.getParameters() != null && Objects.equals(spec.getParameters().getType(), AzureARMParametersFileTypes.Inline)) {
        ...
        AzureTaskNGParameters azureTaskNGParameters = getAzureTaskNGParams(ambiance, stepElementParameters, connectorDTO, passThroughData);
        return azureCreateStepExecutor.executeCreateTask(ambiance, stepElementParameters, azureTaskNGParameters, passThroughData);
    }
    ...
    public TaskChainResponse executeNextLink(AzureCreateStepExecutor azureCreateStepExecutor,
                                             Ambiance ambiance,
