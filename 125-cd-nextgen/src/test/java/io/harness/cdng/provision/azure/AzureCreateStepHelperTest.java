...
public class AzureCreateStepHelperTest extends CategoryTest {
    ...
    @InjectMocks
    private final AzureCreateStepHelper helper = new AzureCreateStepHelper();

    @Before
    public void setUpMocks() {
        MockitoAnnotations.initMocks(this);
        ConnectorInfoDTO connectorInfoDTO =
                ConnectorInfoDTO.builder().connectorConfig(AzureConnectorDTO.builder().build()).build();
    ...
    public void testStartChainLinkWithInlineFilesAndResourceGroup() {
        ...
        ArgumentCaptor<AzureARMTaskNGParameters> taskDataArgumentCaptor = ArgumentCaptor.forClass(AzureARMTaskNGParameters.class);
        helper.startChainLink(azureCreateStepExecutor, getAmbiance(), stepsParameters);
    ...
    public void testHandleGitFetchResponseForBP () {
        ...
        AzureCreateStepConfigurationParameters.AzureCreateStepConfigurationParametersBuilder azureCreateStepConfigurationParametersBuilder =
        ...
        ArgumentCaptor<AzureBlueprintTaskNGParameters> taskDataArgumentCaptor = ArgumentCaptor.forClass(AzureBlueprintTaskNGParameters.class);
        helper.handleGitFetchResponse(azureCreateStepExecutor, getAmbiance(), stepsParameters,azureCreateStepConfigurationParametersBuilder.build(), AzureCreatePassThroughData.builder().build(), gitFetchResponseBuilder.build());
        verify(azureCreateStepExecutor, times(1)).executeCreateTask(any(), any() , taskDataArgumentCaptor.capture(), any());
        assertThat(taskDataArgumentCaptor.getValue()).isNotNull();
    ...
    public void testExecuteNextLinkForARM() throws Exception {
        ...
        GitFetchResponse.GitFetchResponseBuilder gitFetchResponseBuilder = GitFetchResponse.builder();
        gitFetchResponseBuilder.filesFromMultipleRepo(createFetchFilesResultMap(false, true, true));
        helper.executeNextLink(azureCreateStepExecutor, getAmbiance(), stepsParameters, AzureCreatePassThroughData.builder().build(), gitFetchResponseBuilder::build);
        ArgumentCaptor<AzureARMTaskNGParameters> taskDataArgumentCaptor = ArgumentCaptor.forClass(AzureARMTaskNGParameters.class);
    ...
    public void testExecuteNextLinkForInvalidResponse() throws Exception {
        ...
        TaskChainResponse response = helper.executeNextLink(azureCreateStepExecutor, getAmbiance(), stepsParameters, AzureCreatePassThroughData.builder().build(), () -> awsS3BucketResponse);
        assertThat(response.isChainEnd());
    }

    private Map<String, FetchFilesResult> createFetchFilesResultMap(boolean withBP, boolean withParameter, boolean withTemplate) {
        ...
        if (withBP) {
            fetchFilesResultMap.put("bluePrint", fetchFilesResult.files(new ArrayList<>(Arrays.asList(
                    GitFile.builder()
                            .fileContent("bluePrint")
                            .filePath("blueprint.json")
                            .build(),
                    GitFile.builder()
                            .fileContent("AssignContent")
                            .filePath("assign.json").build(),
                    GitFile.builder()
                            .filePath("artifacts/superfile")
                            .fileContent("ArtifactsContent")
                            .build()))).build());
        }
        if (withParameter) {
            fetchFilesResultMap.put("templateFile", fetchFilesResult.files(new ArrayList<>(Arrays.asList(
                    GitFile.builder()
                            .fileContent("templateFile")
                            .filePath("templateFile.json")
                            .build()))).build());
        }
        if (withTemplate) {
            fetchFilesResultMap.put("parameterFile", fetchFilesResult.files(new ArrayList<>(Arrays.asList(
                    GitFile.builder()
                            .fileContent("parameterFile")
                            .filePath("parameterFile.json")
                            .build()))).build());
        }
    ...
    private StepElementParameters createStepForBP (){
        ...
        AzureCreateStepConfigurationParameters.AzureCreateStepConfigurationParametersBuilder configurationParameters = AzureCreateStepConfigurationParameters.builder();
        AzureBluePrintDeploymentSpec.AzureBluePrintDeploymentSpecBuilder azureBluePrintDeploymentSpecBuilder = AzureBluePrintDeploymentSpec.builder();
        azureBluePrintDeploymentSpecBuilder.connectorRef("azureConnector");

        StoreConfigWrapper templateStore =
                StoreConfigWrapper.builder()
                        .spec(GithubStore.builder().paths(ParameterField.createValueField(new ArrayList<>())).connectorRef(ParameterField.createValueField("template-connector-ref")).build())
                        .build();
    ...
    private StepElementParameters createStepForARM(Boolean remoteTemplate, Boolean remoteParameters, Boolean withAzureConnector, String typeOfScope) {
        AzureCreateStepParameters stepParameters = new AzureCreateStepParameters();
        AzureCreateStepConfigurationParameters.AzureCreateStepConfigurationParametersBuilder configurationParameters = AzureCreateStepConfigurationParameters.builder();
        ...
        StoreConfigWrapper fileStoreConfigWrapper =
                StoreConfigWrapper.builder()
                        .spec(GithubStore.builder().paths(ParameterField.createValueField(new ArrayList<>())).connectorRef(ParameterField.createValueField("parameters-connector-ref")).build())
                        .build();
        StoreConfigWrapper templateStore =
                StoreConfigWrapper.builder()
                        .spec(GithubStore.builder().paths(ParameterField.createValueField(new ArrayList<>())).connectorRef(ParameterField.createValueField("template-connector-ref")).build())
        ...
        switch (typeOfScope) {
            case "RESOURCE_GROUP":
                ...
                break;
            case "SUBSCRIPTION":
                AzureSubscritionSpec subscriptionSpec = AzureSubscritionSpec.builder()
                        .subscription(ParameterField.createValueField("subscription"))
            ...
            case "MANAGEMENT_GROUP":
                AzureManagementSpec managementGroupSpec = AzureManagementSpec.builder()
                        .managementGroupId(ParameterField.createValueField("management_group"))
                        .deploymentDataLocation(ParameterField.createValueField("deployment_data_location"))
                        .mode(ParameterField.createValueField("INCREMENTAL"))
                        .build();
                armDeploymentSpec.scope(scopeBuilder.type(AzureScopeTypesNames.MANAGEMENT_GROUP).spec(managementGroupSpec).build());
                break;
            case "TENANT":
                AzureTenantSpec tenantSpec = AzureTenantSpec.builder()
                        .deploymentDataLocation(ParameterField.createValueField("deployment_data_location"))
                        .mode(ParameterField.createValueField("INCREMENTAL"))
                        .build();
