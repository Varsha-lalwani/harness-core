...
public class AzureCreateStepHelperTest extends CategoryTest {
  ...
  public void setUpMocks() {
    ...
    doReturn(connectorInfoDTO).when(cdStepHelper).getConnector("azure-connector", getAmbiance());
    ...
    ConnectorInfoDTO gitConnectorInfoDTO =
        ConnectorInfoDTO.builder()
            ...
            .build();
    doReturn(gitConnectorInfoDTO).when(cdStepHelper).getConnector("parameters-connector-ref", getAmbiance());
    doReturn(gitConnectorInfoDTO).when(cdStepHelper).getConnector("template-connector-ref", getAmbiance());
  ...
  // TODO: Implement when the Harness store is implemented
  ...
  //    helper.startChainLink(azureCreateStepExecutor, getAmbiance(), stepsParameters);
  //    verify(azureCreateStepExecutor, times(1)).executeCreateTask(any(), any(), taskDataArgumentCaptor.capture(),
  //    any()); assertThat(taskDataArgumentCaptor.getValue()).isNotNull(); AzureARMTaskNGParameters
  //    azureTaskNGParameters = taskDataArgumentCaptor.getValue();
  ...
  private StepElementParameters createStepForARM(Boolean remoteTemplate, Boolean remoteParameters, String typeOfScope) {
    ...
    configurationParameters.connectorRef(ParameterField.createValueField("azure-connector"));
    ...
    StoreConfigWrapper harnessParametersInline =
        StoreConfigWrapper.builder().spec(HarnessStore.builder().build()).build();
    StoreConfigWrapper harnessTemplateInline =
        StoreConfigWrapper.builder().spec(HarnessStore.builder().build()).build();

    if (remoteParameters) {
      parameterFileBuilder.setStore(fileStoreConfigWrapper);
    } else {
      parameterFileBuilder.setStore(harnessParametersInline);
    }

    if (remoteTemplate) {
      templateFileBuilder.setStore(templateStore);
    } else {
      templateFileBuilder.setStore(harnessTemplateInline);
    }
    ...
    switch (typeOfScope) {
      ...
      case "TENANT":
        AzureTenantSpec tenantSpec =
            AzureTenantSpec.builder().location(ParameterField.createValueField("deployment_data_location")).build();
