...
public class AzureSshWinrmInstanceSyncPerpetualTaskExecutor implements PerpetualTaskExecutor {
  ...
  private PerpetualTaskResponse executeTask(
    ...
    List<String> azureHosts = getAzureHosts(taskParams);
  ...
  private List<String> getAzureHosts(AzureSshInstanceSyncPerpetualTaskParamsNg taskParams) {
    ...
    AzureHostsResponse azureHostsResponse = azureAsyncTaskHelper.listHosts(
        ...
        infraConfig.getSubscriptionId(), infraConfig.getResourceGroup(), azureOSType, null);
    return azureHostsResponse.getHosts().stream().map(AzureHostResponse::getHostName).collect(Collectors.toList());
  }
