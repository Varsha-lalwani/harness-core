...
public class EcsInstanceSyncPerpetualTaskExecutor implements PerpetualTaskExecutor {
    ...
    @Override
    public PerpetualTaskResponse runOnce(PerpetualTaskId taskId, PerpetualTaskExecutionParams params, Instant heartbeatTime) {
    ...
    public PerpetualTaskResponse executeEcsInstanceSyncTask(PerpetualTaskId taskId,
                                 EcsInstanceSyncPerpetualTaskParams taskParams) {
        List<EcsDeploymentReleaseData> deploymentReleaseDataList =
                getEcsDeploymentReleaseData(taskParams);
        ...
        log.info("Ecs sync nInstances: {}, task id: {}",
    ...
    private EcsDeploymentReleaseData toEcsDeploymentReleaseData(
            EcsDeploymentRelease ecsDeploymentRelease) {
        return EcsDeploymentReleaseData.builder()
                .ecsInfraConfig((EcsInfraConfig) kryoSerializer.asObject(
                        ecsDeploymentRelease.getEcsInfraConfig().toByteArray()))
    ...
    private String publishInstanceSyncResult(
        ...
        try {
            ...
            String errorMsg =
                    format("Failed to publish ECS instance sync result PerpetualTaskId [%s], accountId [%s]",
