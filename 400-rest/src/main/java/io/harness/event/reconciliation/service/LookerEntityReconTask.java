...
public class LookerEntityReconTask implements Runnable {
  LookerEntityReconService lookerEntityReconService;
  @Inject AccountService accountService;
  @Inject private Set<TimeScaleEntity<?>> timeScaleEntities;
  //    @Inject
  //    DeploymentEventProcessor deploymentEventProcessor;
        ...
        if (accountIds.contains(account)) {
          for (TimeScaleEntity timeScaleEntity : timeScaleEntities) {
            ...
            executorService.submit(() -> {
              ...
              //                                     persistentLocker.tryToAcquireLock(Account.class,
              //                                     DATA_MIGRATION_CRON_LOCK_PREFIX + account.getUuid(),
              //                                             Duration.ofSeconds(DATA_MIGRATION_CRON_LOCK_EXPIRY_IN_SECONDS)))
              //                                             {
              //                            if (lock == null) {
              //                                log.error(
              //                                        "Unable to fetch lock for running deployment data migration for
              //                                        account : {}", account.getUuid());
              //                                return;
              //                            }
              //
              //                            if (featureFlagService.isEnabled(
              //                                    FeatureName.CUSTOM_DASHBOARD_ENABLE_CRON_DEPLOYMENT_DATA_MIGRATION,
              //                                    account.getUuid())) {
              //                                log.info("Triggering deployment data migration cron for account : {}",
              //                                account.getUuid()); try {
              //                                deploymentEventProcessor.doDataMigration(account.getUuid(),
              //                                DATA_MIGRATION_INTERVAL_IN_HOURS); } catch (Exception exception) {
              //                                    log.error("Deployment data migration failed for account id : {}",
              //                                    account.getUuid(), exception);
              //                                }
              //                            }
              //                        }
            });
