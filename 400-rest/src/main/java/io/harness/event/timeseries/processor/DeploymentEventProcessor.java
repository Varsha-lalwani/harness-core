...
public class DeploymentEventProcessor implements EventProcessor<TimeSeriesEventInfo> {
  ...
  private static final String insert_statement_migration_parent_table =
      "INSERT INTO DEPLOYMENT_PARENT (EXECUTIONID,STARTTIME,ENDTIME,ACCOUNTID,APPID,TRIGGERED_BY,TRIGGER_ID,STATUS,SERVICES,CLOUDPROVIDERS,ENVIRONMENTS,PIPELINE,DURATION,ARTIFACTS,ENVTYPES,PARENT_EXECUTION,FAILURE_DETAILS,FAILED_STEP_NAMES,FAILED_STEP_TYPES,STAGENAME,ROLLBACK_DURATION, INSTANCES_DEPLOYED, TAGS,PARENT_PIPELINE_ID,WORKFLOWS,CAUSE) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
  ...
  private void insertDataInTimescaleDB(
    ...
    insertStatement.setString(++index, eventInfo.getStringData().get(EventProcessor.CAUSE));
