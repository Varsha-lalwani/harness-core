...
import io.harness.ng.core.dto.UserGroupFilterForUser;
...
public class NGAggregateResource {
  ...
  @POST
  @Path("acl/user/{userIdentifier}/usergroups")
  @ApiOperation(value = "Get Aggregated User Group list by User Id", nickname = "getUserGroupAggregateListByUser")
  public ResponseDTO<PageResponse<UserGroupAggregateDTO>> list(
      @NotNull @PathParam("userIdentifier") String userIdentifier,
      @NotNull @QueryParam(NGCommonEntityConstants.ACCOUNT_KEY) String accountIdentifier,
      @QueryParam(NGCommonEntityConstants.ORG_KEY) String orgIdentifier,
      @QueryParam(NGCommonEntityConstants.PROJECT_KEY) String projectIdentifier, @BeanParam PageRequest pageRequest,
      @QueryParam(NGResourceFilterConstants.SEARCH_TERM_KEY) String searchTerm,
      @QueryParam("userSize") @DefaultValue("6") @Max(20) int userSize,
      @Body UserGroupFilterForUser userGroupFilterForUser) {
    accessControlClient.checkForAccessOrThrow(ResourceScope.of(accountIdentifier, orgIdentifier, projectIdentifier),
    ...
    return ResponseDTO.newResponse(aggregateUserGroupService.listAggregateUserGroupsForUser(
        pageRequest, userGroupFilterForUser.getScopeFilter(), userIdentifier, searchTerm, userSize));
  }
