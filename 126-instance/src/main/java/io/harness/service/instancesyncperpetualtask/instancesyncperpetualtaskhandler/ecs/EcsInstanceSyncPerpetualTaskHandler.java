...
public class EcsInstanceSyncPerpetualTaskHandler extends InstanceSyncPerpetualTaskHandler {
    ...
    @Override
    public PerpetualTaskExecutionBundle getExecutionBundle(InfrastructureMappingDTO infrastructureMappingDTO, List<DeploymentInfoDTO> deploymentInfoDTOList, InfrastructureOutcome infrastructureOutcome) {
    ...
    private EcsDeploymentReleaseData toEcsDeploymentReleaseData(
            InfrastructureMappingDTO infrastructureMappingDTO, EcsDeploymentInfoDTO deploymentInfoDTO,
            InfrastructureOutcome infrastructureOutcome) {
        EcsInfraConfig ecsInfraConfig =
                getEcsInfraConfig(infrastructureMappingDTO, infrastructureOutcome);
    ...
    private EcsInstanceSyncPerpetualTaskParams createEcsInstanceSyncPerpetualTaskParams(
        ...
        return EcsInstanceSyncPerpetualTaskParams.newBuilder()
                ...
                .addAllEcsDeploymentReleaseList(
                        toEcsDeploymentReleaseList(deploymentReleaseData))
                .build();
    }

    private List<EcsDeploymentRelease> toEcsDeploymentReleaseList(
            List<EcsDeploymentReleaseData> deploymentReleaseData) {
        return deploymentReleaseData.stream()
                .map(this::toEcsDeploymentRelease)
                .collect(Collectors.toList());
    }

    private EcsDeploymentRelease toEcsDeploymentRelease(
            EcsDeploymentReleaseData releaseData) {
    ...
    private List<ExecutionCapability> getExecutionCapabilities(
            List<EcsDeploymentReleaseData> deploymentReleaseDataList) {
        Optional<EcsDeploymentReleaseData> deploymentReleaseSample =
                deploymentReleaseDataList.stream().findFirst();
        if (!deploymentReleaseSample.isPresent()) {
            return Collections.emptyList();
        }
        return toEcsInstanceSyncRequest(deploymentReleaseSample.get())
                .fetchRequiredExecutionCapabilities(null);
    }

    private EcsInstanceSyncRequest toEcsInstanceSyncRequest(
            EcsDeploymentReleaseData ecsDeploymentReleaseData) {
